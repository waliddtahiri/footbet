...
export const loadUser = () => (dispatch, getState) => {
    ...
    axios.get('http://localhost:5000/auth/player' + tokenConfig(getState))
        .then(res => dispatch({
            type: USER_LOADED,
            payload: res.data
        }))
...
export const register = ({ username, password }) => dispatch => {
    ...
    axios.post('http://localhost:5000/auth/signup', body, config)
        .then(res => dispatch({
...
export const loginAdmin = ({ username, password }) => dispatch => {
    ...
    axios.post('http://localhost:5000/auth/loginAdmin', body, config)
        .then(res => dispatch({
            type: LOGIN_SUCCESS,
            payload: res.data
        }))
